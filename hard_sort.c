...
void    first_sort(t_list **stack_a, t_list **stack_b, t_sort *sort)
{
    ...
    while(i++ < count_a)
    {
        ...
        {
            if(count_lst(stack_b) > 1 && (*stack_b)->index < mid / 2)
                rr(stack_a, stack_b);
            else if ((*stack_a)->index > sort->mid)
                ra(stack_a);
        }
...
void    find_next(t_list **stack_a, t_list **stack_b, t_sort *sort)
{
    ...
    else if (count_lst(stack_b) > 1 && (*stack_b)->next->index == sort->min && (*stack_a)->next->index == sort->min + 1)
        ss(stack_a, stack_b);
    else if (count_lst(stack_b) == 2 && (*stack_b)->next->index == sort->min && (*stack_a)->next->index == sort->min)
        sb(stack_b);
    else if (ft_lstlast(*stack_b) == sort->min && count_lst(stack_b) > 1)
        rrb(stack_b);
    else
        return ;
...
void    hard_sort(t_list **stack_a, t_list **stack_b)
{
    ...
    while (!(a_is_sorted(stack_a, count)))
    {
